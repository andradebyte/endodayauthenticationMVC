...
export default class QuestionarioModel {
    ...
    try {
        ...
        // **NOVA LÓGICA: Repete perguntas aleatórias se faltar!**
        if (selectedIds.length > 0 && selectedIds.length < batchSize) {
            const deficit = batchSize - selectedIds.length;
            for (let i = 0; i < deficit; i++) {
                const randIdx = Math.floor(Math.random() * selectedIds.length);
                selectedIds.push(selectedIds[randIdx]);
            }
        }

        if (!selectedIds.length) return [];

        // 4) marca no histórico (apenas para as originais, não para repetidas)
        // Obs: Se quiser marcar só os únicos, pode usar new Set aqui, mas a lógica atual mantém histórico normal.
        await Promise.all(
        ...
        // 5) busca perguntas + respostas (incluindo repetidas se necessário)
        ...
        // Agora precisamos garantir que a ordem de rows bata com selectedIds (e repita, se preciso)
        // Como o SQL retorna só únicos, precisamos montar o array completo:
        const rowsById = {};
        rows.forEach(row => { rowsById[row.id] = row; });
        const fullBatch = selectedIds.map(id => rowsById[id]);
